...
#   2. The differential expression analysis (DESeq2::DESeq())
#   3. Extraction of the results (DESeq2::results)
# 
# ------------------------------------------------------------------------
...
# Overrepresentation analysis of only upregulated genes:
gene_lung_up <- rownames(res_lung[res_lung$log2FoldChange >= FCcutoff &
                                 res_lung$padj <= pCutoff, ])                      # Ensembl IDs of upregulated DE lung genes.
gene_blood_up <- rownames(res_blood[res_blood$log2FoldChange >= FCcutoff &
                                   res_blood$padj <= pCutoff, ])                   # Ensembl IDs of upregulated DE blood genes.
universe_lung_up <- rownames(res_lung[res_lung$log2FoldChange >= FCcutoff, ])      # Ensembl IDs of all upregulated genes measured.
universe_blood_up <- rownames(res_blood[res_blood$log2FoldChange >= FCcutoff, ])   # Ensembl IDs of all upregulated genes measured.



ego_lung_up <- enrichGO(
  gene          = gene_lung_up,
  universe      = universe_lung_up,
  OrgDb         = org.Mm.eg.db,
  ont           = "BP",         # Choose BP, MF, CC, or ALL
  keyType       = "ENSEMBL",
  pAdjustMethod = "BH",         # Benjamini-Hochberg adjustment
...
ego_blood_up <- enrichGO(
  gene          = gene_blood_up,
  universe      = universe_blood_up,
  OrgDb         = org.Mm.eg.db,
  ont           = "BP",         # Choose BP, MF, CC, or ALL
  keyType       = "ENSEMBL",
  pAdjustMethod = "BH",         # Benjamini-Hochberg adjustment
...
dotplot(ego_lung_up, showCategory=10) + ggtitle("Top 10 Upregulated Lung GO Terms by Biological Process")
dotplot(ego_blood_up, showCategory=10) + ggtitle("Top 10 Upregulated Blood GO Terms by Biological Process")
